...
const int ROW = 6;
const int COLUMN = 7;
void printBoard(const char board[][COLUMN]); //게임판(현황) 출력
void dropDisk(char board[][COLUMN], int dropState[], const int drop, const char color);//디스크를 입력
int minDropColumn(const int dropState[]);//사용자가 입력가능한 최소 열
int maxDropColumn(const int dropState[]);//사용자가 입력가능한 최대 열
bool gameSet(const char board[][COLUMN],const int dropState[], int* win);//게임 승패, 무승부, 게임중 판별
...
void printBoard(const char board[][COLUMN]) {
	for (int i = 0;i < ROW;i++) {
		cout << "|";
...
void dropDisk(char board[][COLUMN], int dropState[], const int drop, const char color) {
	board[ROW - 1 - dropState[drop]][drop] = color; //dropState를 이용하여 디스크를 입력할 장소 구하기
	dropState[drop]++;
}

bool gameSet(const char board[][COLUMN], const int dropState[], int* win) {
	//승패 판별
