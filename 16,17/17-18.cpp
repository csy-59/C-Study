...
bool search(int grid[][9], int freeCell[][2], int k, int num);
...
bool search(int grid[][9])
{
	...
	cout << numberOfFreeCells << endl;
	
	return search(grid, freeCellList, k, numberOfFreeCells);
	/*
	while (true)
	{
...
bool search(int grid[][9],  int freeCell[][2], int k, int num) {
	if ((k == num) && (grid[freeCell[k - 1][0]][freeCell[k - 1][1]] != 0))
		return true;
	else {
		int i = freeCell[k][0], j = freeCell[k][1];
		grid[i][j] = 1;
		bool isFound = false;
		while (grid[i][j]<10) {
			if (isValid(i, j, grid)) {
				isFound = search(grid, freeCell, k+1, num);
		...
		if ((grid[i][j] == 0)) {
			//cout << k << " " << grid[i][j]<<"back" << endl;
			return false;
		}
		else if (k < num)
			search(grid, freeCell, k + 1, num);
	}
}